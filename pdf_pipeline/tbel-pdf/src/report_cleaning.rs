...
pub fn clean_report_tables(result: &ProcessingResult) -> Vec<CleanedTable> {
    result
        ...
        .filter_map(|table| {
            ...
            let value_count = header.len() - 1;
            let num_dates = original_date_headers.len();
            ...
            let value_cols: Vec<usize> = (1..=headers.len() - 1)
                .take_while(|&i| i < header.len())
                .collect();
            ...
            let cleaned_rows: Vec<Vec<String>> = rows
                .iter()
                .skip(data_row_start)
                .filter_map(|row| {
                    ...
                    cleaned.push(code);
                    cleaned.extend(value_cols.iter().map(|&col_index| {
